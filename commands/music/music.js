...
const handleVideo = async (video, message, voiceChannel, playlist = false) => {
  ...
  if (!queue.has(message.guild.id)) {
    ...
    if (
      queue.get(message.guild.id).songs.length >=
      message.settings.maxqueuelength
    )
...
function play(guild, song) {
  ...
  const dispatcher = queue
    ...
    .connection.play(
      ytdl(
        song.url,
        { quality: 'lowest', filter: 'audioonly' },
        { passes: 3, volume: guild.voiceConnection.volume || 0.2 }
      )
    )
    .on('end', () => {
      if (!serverQueue.loop) {
  ...
  const embed = new MessageEmbed()
    ...
    .setColor(
      guild.member(guild.client.user.id).roles.highest.color || 0x00ae86
    );
